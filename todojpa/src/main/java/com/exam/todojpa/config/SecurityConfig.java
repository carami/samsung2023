...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/addUser").permitAll().antMatchers("/api/**")
				.hasRole("USER").antMatchers("/todo.html").hasRole("USER") // ignoring되어 있어서 권한이 없어도 접근된다. 이경운 html에서특정
																			// api를 호출한 후 403 오류가 발생하면redirect하도록 한다.
				.antMatchers("/admin/api/**").hasRole("ADMIN").anyRequest()
...
//	                .successHandler(new CustomneAuthenticationSuccessHandler())
				.permitAll().and().logout().logoutUrl("/logout").logoutSuccessUrl("/userregform.html").permitAll()
